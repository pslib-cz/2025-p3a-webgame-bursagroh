...
import { CHUNK_SIZE, VIEW_DISTANCE_HORIZONTAL, VIEW_DISTANCE_VERTICAL } from '../../../constants/map'
...
const CitySVG = () => {
    ...
    const chunks = getChunkList(player.positionX, player.positionY, VIEW_DISTANCE_HORIZONTAL, VIEW_DISTANCE_VERTICAL, CHUNK_SIZE)

    return (
        <SVGDisplay centerX={player.positionX} centerY={player.positionY} className={styles.city}>
            {chunks.map((chunk) => <Chunk key={`x:${chunk.x};y:${chunk.y}`} x={chunk.x} y={chunk.y} size={CHUNK_SIZE} />)}
